...
Im Folgenden bezeichne $\Delta$ ein fest gewähltes Fundamentalsystem des Wurzelsystems $\Phi$ im \euklid ischen Vektorraum $E$.
...
\begin{proof}
  ...
  Es gilt $\hgt(\beta - \alpha_{k}) = k - 1$ und nach Induktionsvoraussetzung folgt $\alpha_1 + \dots + \alpha_{k-1} \in \Phi^+$ für alle $i \in \{1,\dots,k-1\}$.
...
\begin{proof}
  ...
  Daher existiert ein $\gamma \neq \beta$ mit $k_\gamma > 0$.

  Aufgrund der Linearität von $\sigma_\alpha$ besitzt dann auch $\sigma_\alpha(\beta)$ mindestens einen positiven Linearfaktor und damit ist nach \hyperref[it:B2]{(B2)} auch $\sigma_\alpha(\beta)$ positiv.
  Des Weiteren ist $\sigma_\alpha(\beta)$ nicht proportional zu $\alpha$, da sonst $\beta = -\alpha$ gelten würde, was im vorangehenden Absatz bereits ausgeschlossen wurde.
...
\begin{proof}
  ...
  Wiederum impliziert \ref{lem:permute} die Gleichung $\beta_s = \alpha_s$, da sonst andernfalls $\sigma_s\beta_s$ positiv sein müsste.
