...
Ziel dieses Abschnittes ist es zu beweisen, dass die durch die Gruppe $\W$ auf der Menge der Fundamentalsysteme induzierte Gruppenoperation einfach transitiv ist.
Wir beginnen mit einer für beliebige Gruppenoperationen definierten Eigenschaft.

\begin{defn}
  Eine Gruppe $G$ operiere auf einer Menge $X$.
  Die Gruppenoperation $\circ \colon G \times X \to X$ heißt \emph{einfach transitiv}, wenn für alle $x,y \in X$ genau ein $g \in G$ existiert, sodass $g \circ x = y$ gilt. 
\end{defn}
...
\begin{proof}
  ...
  (d):
  Wir stellen zunächst fest, dass aufgrund der Aussage (c), die zu zeigende Gleichheit in dem Sinne wohldefiniert ist, dass $\sigma(\Delta(\gamma))$ als Bild eines Fundamentalsystems wieder ein Fuxindamentalsystem darstellt.
...
\begin{thm}
  ...
  \begin{enumerate}[(a)]
    ...
    \item Ist $\sigma \in \W$ und gilt $\sigma(\Delta) = \Delta$, so folgt $\sigma = \mathds{1}$.
      Also operiert $\W$ einfach transitiv auf der Menge der Fundamental\-systeme.
  \end{enumerate}
\end{thm}
\end{samepage}
...
\begin{proof}
  ...
  Es soll nun gezeigt werden, dass hieraus bereits die einfache Transitivität der Gruppenoperation folgt.
  ...
  Also operiert $W$ sogar einfach transitiv auf der Menge der Fundamentalsysteme.
\end{proof}
