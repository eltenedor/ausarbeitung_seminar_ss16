...
Ziel dieses Abschnittes ist es nun zu beweisen, dass die durch die Gruppe $\W$ auf der Menge der Fundamentalsysteme induzierte Gruppenoperation scharf transitiv ist.
...
\begin{lem}
  ...
  Sei $\Phi$ ein Wurzelsystem in $E$. Dann ist die entsprechende \weyl\hyp{}Gruppe endlich. 
\end{lem}
...
\begin{thm}
  ...
  Es sei $\Delta$ ein Fundamentalsystem des Wurzelsystems $\Phi$ in $E$ und zugehöriger \weyl\hyp{}Gruppe $\W$.
...
\begin{proof}
  ...
  \begin{align*}
    (\sigma(\gamma), \delta) 
    &\geq (\sigma_\alpha \sigma(\gamma), \delta) &&\text{aufgrund der Maximalität von $(\sigma(\gamma),\delta)$} \\
  ...
  Wir wählen nun ein $\varepsilon > 0$ und $\gamma' \in E$ möglichst nahe bei $\gamma$, sodass $(\gamma', \alpha) = \varepsilon > 0$ ist und $|(\gamma', \beta)| > \varepsilon$ ($\ast$) für alle $\beta \neq \pm \alpha$ gilt.
  ...
  Nach Konstruktion $\gamma'$ regulär.
  Zudem ist $\alpha$ unzerlegbar.
  Angenommen, $\alpha$ wäre zerlegbar.
  Dann existieren $\beta_1, \beta_2 \in \Phi^+(\gamma')$ mit $\alpha = \beta_1 + \beta_2$.
  Mit der Bilinerität des Skalarproduktes folgt dann jedoch unter Berücksichtigung von ($\ast$)
  \begin{displaymath}
    \varepsilon 
    = (\gamma', \alpha) 
    = (\gamma', \beta_1 + \beta_2) 
